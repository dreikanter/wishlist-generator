...
class Wishlist:
    ...
    class Conf:
        def __init__(self, conf_file, section_name):
            try:
                ...
                self.sourceFile = self._conf.get(section_name, 'source_file')
                self.dataFile = self._conf.get(section_name, 'data_file')
                self.tmpPath = self._conf.get(section_name, 'tmp_path')
                self.imagePath = self._conf.get(section_name, 'image_path')
                self.identifyCmd = self._conf.get(section_name, 'identify_cmd')
                self.mogrifyCmd = self._conf.get(section_name, 'mogrify_cmd')
                self.csvDelimiter = self._conf.get(section_name, 'csv_delimiter')
            except:
                print "Error reading configuration file '%s' (section: '%s')" % (conf_file, section_name)
                raise

    def __init__(self, conf_file, conf_section = 'wishlist-builder'):
        self._conf = WishlistParocessor.Conf(conf_file, conf_section)

    def generate(self):
        print self._conf
        return

    def read_source_file(self, source_file):
    ...
    def get_image_size(self, images_list):
        '''Returns a collection of [width, height] for a set of image files'''
        try:
            result = []
            for line in os.popen(self._conf.identifyCmd % ' '.join(images_list)).readlines():
                parts = line.split(',')
                if len(parts) != 3: continue
    ...
    def save_data(self, csv_file, data):
        ...
        try:
            writer = csv.writer(open(csv_file, 'wb'), delimiter = self._conf.csvDelimiter, quoting = csv.QUOTE_MINIMAL)
    ...
    def load_data(self, csv_file):
        ...
        try:
            result = []
            reader = csv.reader(open(csv_file, 'rb'), delimiter = self._conf.csvDelimiter)
    ...
    def download_images(self, data):
        ...
        for item in data:
            ...
            save_as = os.path.join(self._conf.imagePath, str(counter) + ext)
            print '%s -> %s' % (item['image_url'], save_as)
            self.download(item['image_url'], save_as)
            item['image_id'] = counter
            result.append(save_as)
        return result

    def scale_images(self, max_width, max_height):
        os.popen(self._conf.mogrifyCmd % (max_width, max_height, os.path.join(self._conf.imagePath, '*')))
        return



Wishlist('build.conf').generate()
